...
// NOTE: this was copied out of 6502bench SourceGen v1.8.5.  It has been modified for use here.
...
namespace FileConv.Code {
    ...
    public class OmfSegment {
        ...
        private static int GetWord(byte[] data, int offset, int width, bool isBigEndian) {
            return (int)RawData.GetUWord(data, offset, width, isBigEndian);
        }
        ...
        public static ParseResult ParseHeader(byte[] data, int offset, bool parseAsLibrary,
            ...
            int blkByteCnt = GetWord(data, offset + 0x00, 4, false);
            newSeg.ResSpc = GetWord(data, offset + 0x04, 4, false);
            newSeg.Length = GetWord(data, offset + 0x08, 4, false);
            newSeg.LabLen = data[offset + 0x0d];
            int numLen = data[offset + 0x0e];
            newSeg.BankSize = GetWord(data, offset + 0x10, 4, false);
            int numSex, dispName;
            if (newSeg.Version == SegmentVersion.v0_0) {
                newSeg.Org = GetWord(data, offset + 0x14, 4, false);
                newSeg.Align = GetWord(data, offset + 0x18, 4, false);
                ...
                newSeg.BankSize = GetWord(data, offset + 0x10, 4, false);
                newSeg.Org = GetWord(data, offset + 0x18, 4, false);
                newSeg.Align = GetWord(data, offset + 0x1c, 4, false);
                numSex = data[offset + 0x20];
                newSeg.LcBank = data[offset + 0x21];    // v1.0 only
                newSeg.SegNum = GetWord(data, offset + 0x22, 2, false);
                newSeg.Entry = GetWord(data, offset + 0x24, 4, false);
                dispName = GetWord(data, offset + 0x28, 2, false);
                newSeg.DispData = GetWord(data, offset + 0x2a, 2, false);
            }
            ...
            if (newSeg.Version == SegmentVersion.v2_0 && dispName > 0x2c) {
                ...
                newSeg.TempOrg = GetWord(data, offset + 0x2c, 4, false);
            }
            ...
            if (newSeg.Version <= SegmentVersion.v1_0) {
                ...
                kindWord = GetWord(data, offset + 0x14, 2, false);
            ...
            if (newSeg.Version >= SegmentVersion.v2_0) {
                ...
                newSeg.AddRaw("KIND", GetWord(data, offset + 0x14, 2, false), 2,
                    KindToString(newSeg.Kind) + attrStr);
                newSeg.AddRaw("undefined", GetWord(data, offset + 0x16, 2, false), 2,
                    string.Empty);
            } else {
                newSeg.AddRaw("undefined", GetWord(data, offset + 0x14, 4, false), 4,
                    string.Empty);
            }
