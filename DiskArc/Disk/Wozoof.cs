...
namespace DiskArc.Disk {
    /// <summary>
    /// WOZ/MOOF disk image format.
    /// </summary>
    /// <remarks>
    /// <para>This format is designed to hold low-level captures of Apple 5.25" and 3.5"
    ...
    /// floppy disk write protection.  The flag is disregarded here.</para>
    ...
    /// </remarks>
    public class Wozoof : IDiskImage, INibbleDataAccess, IMetadata {
        // DSDD 3.5" disk images are about 1.2MB.  As flux images they'd be about 6x(?) larger.
        ...
        protected const int MIN_FILE_LEN = 256;  // too small to hold signature/INFO/TMAP
        protected const int MAX_FILE_LEN = 32 * 1024 * 1024;

        private const int HEADER_LEN = 12;
        private const int CHUNK_HDR_LEN = 8;
        protected static readonly byte[] SIGNATURE_W1 = new byte[8] {
            0x57, 0x4f, 0x5a, 0x31, 0xff, 0x0a, 0x0d, 0x0a      // WOZ1
        };
        protected static readonly byte[] SIGNATURE_W2 = new byte[8] {
            0x57, 0x4f, 0x5a, 0x32, 0xff, 0x0a, 0x0d, 0x0a      // WOZ2
        };
        protected static readonly byte[] SIGNATURE_M = new byte[8] {
        ...
        public virtual NibCharacteristics Characteristics => sCharacteristics;

        public MediaKind DiskKind { get; private set; }

        //
        // WOZ/MOOF-specific properties.
        //

        public enum WozKind { Unknown = 0, Woz1 = 1, Woz2 = 2 }

        /// <summary>
        /// File structure revision (1 or 2).  Determines how some of the chunks are parsed.
        /// MOOF has the same structure as Woz2.
        /// </summary>
        public WozKind FileRevision { get; private set; } = WozKind.Unknown;

        /// <summary>
        /// True if this is a MOOF file.
        /// </summary>
        ...
        /// <summary>
        /// INFO chunk data holder.  Actual object will be Woz_Info or Moof_Info.
        /// </summary>
        protected Wozoof_Info Info { get; set; }

        /// <summary>
        /// META data holder.  Will be null if the disk image doesn't have a META chunk.
        /// </summary>
        protected Wozoof_Meta? MetaChunk { get; set; }
        ...
        /// </summary>
        protected Wozoof(Stream stream, AppHook appHook) {
            mDataStream = stream;
            mAppHook = appHook;
            Info = new Wozoof_Info();
        }
        ...
        protected virtual void Dispose(bool disposing) {
            ...
            if (disposing) {
                ...
                mAppHook.LogW("GC disposing Wozoof, FileSystem=" + Contents);
                Debug.Assert(false, "GC disposing Wozoof, dirty=" + IsDirty +
                    " created:\r\n" + mCreationStackTrace);
            }
        }
#if DEBUG
        ...
        public void Flush() {
...
#if DEBUG
            {
                ...
                using (Wozoof verify = new Wozoof(newFile, mAppHook)) {
                    verify.Parse();
                    if (verify.IsDubious) {
                        // If previous contents were dubious, we'd be in read-only mode.  Looks
                        // like we broke something.
                        throw new DAException("New WOZ/MOOF failed verification");
                    }
                }
            }
#endif
        ...
        protected void DoCreateDisk525(Stream stream, uint numTracks, SectorCodec codec,
            ...
            mBaseData = tmpBuf;
            mBaseLength = (int)tmpStream.Length;
            ChunkAccess = new GatedChunkAccess(new NibbleChunkAccess(this, codec));
        }

        protected void DoCreateDisk35(Stream stream, MediaKind mediaKind, int interleave,
        ...
        private void WriteHeaderAndInfo(byte[] buffer) {
            ...
            if (IsMoof) {
                Array.Copy(SIGNATURE_M, buffer, SIGNATURE_M.Length);
            } else if (FileRevision == WozKind.Woz1) {
                Array.Copy(SIGNATURE_W1, buffer, SIGNATURE_W1.Length);
            } else {
                Array.Copy(SIGNATURE_W2, buffer, SIGNATURE_W2.Length);
            }
        ...
        protected void Parse() {
            ...
            if (RawData.CompareBytes(mBaseData, SIGNATURE_W1, SIGNATURE_W1.Length)) {
                FileRevision = WozKind.Woz1;
            } else if (RawData.CompareBytes(mBaseData, SIGNATURE_W2, SIGNATURE_W2.Length)) {
                FileRevision = WozKind.Woz2;
            } else if (RawData.CompareBytes(mBaseData, SIGNATURE_M, SIGNATURE_M.Length)) {
                FileRevision = WozKind.Woz2;
                IsMoof = true;
            } else {
                throw new InvalidDataException("Incompatible data stream");
            ...
            if (IsMoof) {
                Moof_Info newInfo = new Moof_Info(mBaseData, INFO_CHUNK_START);
                Info = newInfo;
                switch (newInfo.DiskType) {
                    case 1:
                    ...
                    case 4:
                        ...
                        break;
                    default:
                        throw new InvalidDataException("Unknown disk type: " + newInfo.DiskType);
                }
                if (newInfo.FluxBlock != 0 && newInfo.LargestFluxTrack != 0) {
                    Notes.AddI("Found FLUX data, writing will be disabled");
                    HasFlux = true;
                }
            } else {
                Woz_Info newInfo = new Woz_Info(mBaseData, INFO_CHUNK_START);
                Info = newInfo;
                if (newInfo.DiskType != 1 & newInfo.DiskType != 2) {
                    throw new InvalidDataException("Unknown disk type: " + newInfo.DiskType);
                }
                if (newInfo.Version > 1) {
                    if (newInfo.DiskSides != 1 && newInfo.DiskSides != 2) {
                        throw new InvalidDataException("Unexpected number of sides: " +
                            newInfo.DiskSides);
                    }
                }
                if (newInfo.DiskType == 1) {
                    DiskKind = MediaKind.GCR_525;
                } else {
                    if (newInfo.Version > 1 && newInfo.DiskSides == 1) {
                        DiskKind = MediaKind.GCR_SSDD35;
                    } else {
                        DiskKind = MediaKind.GCR_DSDD35;
                    }
                }
                if (newInfo.Version >= 3) {
                    if (newInfo.FluxBlock != 0 && newInfo.LargestFluxTrack != 0) {
                        Notes.AddI("Found FLUX data, writing will be disabled");
                        HasFlux = true;
                    }
            ...
            if (metaChunkOffset >= 0) {
                MetaChunk = Wozoof_Meta.ParseMETA(mBaseData, metaChunkOffset, metaChunkLength,
                    mAppHook);
            }
        }
        ...
        public bool GetTrackBits(uint trackNum, uint trackFraction,
            ...
            switch (DiskKind) {
                ...
                case MediaKind.GCR_DSDD35:
                    ...
                    if (!IsMoof) {
                        Woz_Info wozinfo = (Woz_Info)Info;
        ...
        private byte[]? FluxToBits(int offset, int count, out int bitCount) {
            ...
            int bitTime = IsMoof ?
                ((Moof_Info)Info).OptimalBitTiming : ((Woz_Info)Info).OptimalBitTiming;
        ...
        public void AddMETA() {
            CheckAccess();
            if (MetaChunk == null) {
                MetaChunk = Wozoof_Meta.CreateMETA(IsMoof);
            }
        }
        ...
        public List<MetaEntry> GetMetaEntries() {
            ...
            if (IsMoof) {
                ...
                if (((Woz_Info)Info).Version > 1) {
                    foreach (MetaEntry met in Woz_Info.sInfo2List) {
                        entries.Add(new MetaEntry(INFO_PFX + met.Key, met.ValueType,
                            met.Description, met.ValueSyntax, met.CanEdit, false));
                    }
                }
                if (((Woz_Info)Info).Version > 2) {
            ...
            if (MetaChunk != null) {
                ...
                MetaEntry[] stdEntries =
                    IsMoof ? Wozoof_Meta.sStandardMoofEntries : Wozoof_Meta.sStandardWozEntries;
        ...
        public void SetCreator(string value) {
            if (IsMoof) {
                ((Moof_Info)Info).Creator = value;
            } else {
                ((Woz_Info)Info).Creator = value;
            }
        }
