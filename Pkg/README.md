...
updating version numbers and running a final set of tests on the app.
 ...
 1. Update the version number in `AppCommon/GlobalAppVersion.cs`.  This
    changes the version number displayed by the applications.
 ...
 3. Update the version number in `Pkg/make-dist.py`.  This determines the
    version number used in the filenames of distribution packages.
 4. Do a full build of the project in Visual Studio, in Debug mode.
 5. Launch the GUI application (`cp2_avalonia` project, built as
 ...
 6. Run the command-line tests: `cp2 debug-test`, `cp2 debug-test-da`,
...
Finally, build the applications and submit the changes.
 ...
 9. Run `Pkg/make-dist.py` from the top of the source tree.  This builds the
