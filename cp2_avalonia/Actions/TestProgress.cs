...
namespace cp2_avalonia.Actions
{
        ...
        : WorkProgressViewModel.IWorker
    {
        ...
        public class Failure(IFileEntry entry, FilePart part)
        {
            public IFileEntry Entry { get; private set; } = entry;
            public FilePart Part { get; private set; } = part;
        }

        /// <summary>
        /// List of results.  If all goes well, the list will be empty.
        /// </summary>
        public List<Failure>? FailureResults { get; private set; }
        ...
        public object DoWork(BackgroundWorker bkWorker)
        {
            ...
            {
                TestArchive(arc, selected, EnableMacOSZip, bkWorker, failures);
            }
            else
            {
                IFileSystem fs = (IFileSystem)archiveOrFileSystem;
                TestFileSystem(fs, selected, bkWorker, failures);
        ...
        {
            var facts = new CallbackFacts(CallbackFacts.Reasons.Progress,
                entry.FullPathName, entry.DirectorySeparatorChar)
            {
                ProgressPercent = perc,
                Part = part
            };
            ProgressUtil.HandleCallback(facts, "test",
                new ProgressUtil.PersistentChoices(), null, bkWorker);
            ...
            {
                AppLog.E("Test files: unexpected error", ex);
                Debug.Assert(false);
                return false;
            }
            ...
            BackgroundWorker bkWorker, List<Failure> failures)
        {
            for (var i = 0; i < selected.Count; i++)
            {
                ...
                var perc = (i * 100) / selected.Count; // approximate
