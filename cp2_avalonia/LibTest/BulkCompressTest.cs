...
namespace cp2_avalonia.LibTest {
    ...
    internal class BulkCompressTest {
        ...
        private readonly byte[] mSrcBuf = new byte[BUFFER_SIZE];
        private readonly byte[] mCompBuf = new byte[BUFFER_SIZE * 2];    // oversized to allow expansion
        private readonly byte[] mExpBuf = new byte[BUFFER_SIZE];

        private readonly BackgroundWorker mWorker;
        private readonly CompressionFormat mFormat;
        private readonly string mCodecName;
        private readonly bool mVerbose = false;
        ...
        /// <param name="appHook">Application hook.</param>
        ...
        private void PrintSummary() {
            ...
            var perc = (mTotalCompLen / (double)mTotalSrcLen) * 100.0;
        ...
        private void TestDiskImage(FileStream fileStream, FileKind kind, SectorOrder orderHint,
            ...
            switch (diskImage.Contents)
            {
                case IFileSystem fs:
                {
                    PrintLogLine("Processing disk image...");
                    fs.PrepareFileAccess(true);
                    IFileEntry rootDir = fs.GetVolDirEntry();
                    TestDiskFiles(fs, rootDir, string.Empty);
                    break;
                }
                case IMultiPart contents:
                {
                    foreach (Partition part in contents) {
        ...
        private void TestArchive(FileStream fileStream, FileKind kind, AppHook appHook) {
            ...
            foreach (IFileEntry ent in archive) {
                ...
                if (ent.HasRsrcFork)
                {
                    using var stream = archive.OpenPart(ent, FilePart.RsrcFork);
                    mForkCount++;
